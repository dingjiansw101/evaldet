...
outpath = r'G:\voc_eval\GFformatPascal'
def parse_rec(filename):
    ...
    newname = basefilename + '.txt'
    outdir = os.path.join(outpath, newname)
    ...
    for obj in tree.findall('object'):
        ...
        cls = obj_struct['name']
        diff = obj_struct['difficult']
        x1, y1, x2, y2, x3, y3, x4, y4 = (int(bbox.find('xmin').text), int(bbox.find('ymin').text),
                                  int(bbox.find('xmax').text), int(bbox.find('ymin').text),
                                  int(bbox.find('xmax').text), int(bbox.find('ymax').text),
                                  int(bbox.find('xmin').text), int(bbox.find('ymax').text) )
        outline = (str(x1) + ' ' + str(y1) + ' ' + str(x2) + ' ' + str(y2) + ' ' + str(x3) +
                        ' ' + str(y3) + ' ' + str(x4) + ' ' + str(y4) + ' ' + cls)
        if (diff):
            outline = outline + ' ' + '1'
        f_out.write(outline + '\n')
    return objects
